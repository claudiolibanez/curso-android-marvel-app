...
//            val uiStateSuccess = detailViewModel.uiState.value as DetailViewModel.UiState.Success
            ...
            assertEquals(2, categoriesParamList.size)
            assertEquals(
                R.string.details_comics_category,
                categoriesParamList[0].categoryStringResId
            )
            assertEquals(
                R.string.details_events_category,
                categoriesParamList[1].categoryStringResId
            )
        }
...
//            val uiStateSuccess = detailViewModel.uiState.value as DetailViewModel.UiState.Success
            ...
            assertEquals(1, categoriesParamList.size)
            assertEquals(
                R.string.details_comics_category,
                categoriesParamList[0].categoryStringResId
            )
...
//            verify(uiStateObserver).onChanged(isA<DetailViewModel.UiState.Error>())
    ...
    fun `should notify favorite_uiState with filled favorite icon when current icon is unchecked`()
        = runTest {
            ...
            val uiState = detailViewModel.favorite.state.value
                    as FavoriteUiActionStateLiveData.UiState.Icon
